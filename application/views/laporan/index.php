...
<div class="row">
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <ul class="nav justify-content-center nav-tabs" id="myTab" role="tablist">
                    ...
                    <li class="nav-item">
                        <a class="nav-link text-secondary" id="contact-tab" data-toggle="tab" href="#pending" role="tab" aria-controls="pending" aria-selected="false"><i class="fa fa-clock"></i> Pending (<span class="total-laporan-pending">0</span>)</a>
                    </li>
                    <li class="nav-item">
                ...
                <div class="tab-content" id="myTabContent"><br>
                    ...
                    <div class="tab-pane fade" id="pending" role="tabpanel" aria-labelledby="contact-tab">
                        <div class="row list-laporan-pending"></div>
                    </div>
                </div>
            </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form class="form-group frmUpdateStatusLaporan">
                    ...
                    <div class="modal-body">
                        <div class="row  mx-sm-3 mb-2 justify-content-between status">
                    ...
                    <div class="modal-footer">
                        ...
                        <input type="hidden" name="tanggal_fifo" class="tanggalFifo">     
    ...
    <script>
                ...
                if (data.status) {
                    ...
                    for (i = 0; i < data.item.length; i++) {
                        ...
                        html +=
                                ...
                                '<div class="card-body"><div class="small text-' + bgstatus + '"><i class="fa fa-' + ikonstatus + '"></i> ' + data.item[i].status_laporan + '</div>' +
                                '<span class="small"><i class="far fa-dot-circle"></i> ' + data.item[i].kategori + '</span>' +
                                '<p class="card-text font-weight-bold">' + data.item[i].nama_laporan +
                                '<div class="small">' + data.item[i].deskripsi + '</div>' +
                                '</p>' +
                                '<div class="small"><i class="fas fa-fw fa-map-marker-alt"></i> ' + data.item[i].lokasi + '</div>' +
                                '</div>' +
                                '</div>' +
                                '</div>';
                    }
                    ...
                    $('.list-semua-laporan').html("<div class='text-center col'>No data available</div>");
                }
        ...
        function list_laporan_verifikasi() {
            $.ajax({
                ...
                if (data.status) {
                    ...
                    $('.list-laporan-verifikasi').html("<div class='text-center col'>No data available</div>");
                }
        ...
        function list_laporan_proses() {
            $.ajax({
                ...
                if (data.status) {
                    ...
                    $('.list-laporan-proses').html("<div class='text-center col'>No data available</div>");
                }
        ...
        function list_laporan_follow_up() {
            $.ajax({
                ...
                if (data.status) {
                    ...
                    $('.list-laporan-follow-up').html("<div class='text-center col'>No data available</div>");
                }
        ...
        function list_laporan_selesai() {
            $.ajax({
                ...
                if (data.status) {
                    ...
                    $('.list-laporan-selesai').html("<div class='text-center col'>No data available</div>");
                }
        ...
        function list_laporan_pending() {
            $.ajax({
                type: 'GET',
                url: '<?php echo base_url() ?>services/laporan/list_laporan_pending',
                dataType: 'json'
            }).then(function (data) {
                if (data.status) {
                    ...
                    $('.list-laporan-pending').html(html);
                } else {
                    $('.list-laporan-pending').html("<div class='text-center col'>No data available</div>");
                }
                $('.list-laporan-pending').html(html);
                $('.total-laporan-pending').text(data.total_laporan_pending);
            });
        }
        ;
            ...
            $.ajax({
                ...
                if (data.status) {
                    ...
                    if (data.item.status_laporan == "<?= VERIFIKASI ?>") {
                        ...
                        progress_status = "25";
                        $('.tanggalFifo').val(data.item.tanggal_lapor);
                        $('.idLaporan').val(data.item.id);
                        if ('<?= $data_user->level ?>' == '<?= KASATPEL ?>') {
                            $('.btnUpdateStatus').html('<input type="submit" name="submit" value="Verifikasi" class="btn-sm btn-primary mb-2 btnUpdateStatusLaporan"> - <input type="submit" name="submit" value="Pending" class="btn-sm btn-warning mb-2 btnUpdateStatusLaporan">');
                        }
                        
                    } else if (data.item.status_laporan == "<?= PROSES ?>") {
                        progress_status = "50";
                        
                        $('.tanggalFifo').val(data.item.tanggal_verifikasi);
                        
                        if ('<?= $data_user->level ?>' == '<?= PETUGAS ?>') {
                            $('.btnUpdateStatus').html('<input type="submit" name="submit" value="Proses" class="btn-sm btn-primary mb-2 btnUpdateStatusLaporan">');
                        }
                        ...
                        if ('<?= $data_user->level ?>' == '<?= PETUGAS ?>') {
                            ...
                            $('.btnUpdateStatus').html('<input type="submit" name="submit" value="Selesai" class="btn-sm btn-primary mb-2 btnUpdateStatusLaporan">');
                        }
                        
                    } else if (data.item.status_laporan == "<?= SELESAI ?>") {
                        progress_status = "100";
                        tindakan = '<tr><th colspan="3" class="text-center bg-gray-300">Tindakan Laporan</th></tr>' +
                    ...
                    } else if (data.item.status_laporan == "<?= PENDING ?>") {
                        progress_status = "0";
                        $('.status').hide();
                        $('.progressStatus').hide();
                        $('.idLaporan').val(data.item.id);
                        if ('<?= $data_user->level ?>' == '<?= KASATPEL ?>') {
                            $('.btnUpdateStatus').html('<input type="submit" name="submit" value="Verifikasi" class="btn-sm btn-primary mb-2 btnUpdateStatusLaporan">');
                        }
                    }
                    ...
                    html +=
                            ...
                            '<tr><td>Pelapor</td><td>:</td><td><a href="user/' + data.item.user_id + '">' + data.item.nama_lengkap + '</a> (RT/RW ' + data.item.rt + '/' + data.item.rw + ')</td></tr>' +
                            '<tr><td>No. Telpon Pelapor</td><td>:</td><td>' + data.item.nomor_hp + '</td></tr>' +
                            '<tr><td>Email Pelapor</td><td>:</td><td>' + data.item.email + '</td></tr>' +
